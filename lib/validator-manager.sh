...
install_web3signer() {
    ...
    local network_options=("mainnet" "hoodi")
    ...
    create_web3signer_env_file "$staging_dir" "$postgres_password" "$keystore_password" "$selected_network" "$keystore_location" "$web3signer_port"
    create_web3signer_compose_files "$staging_dir"
    create_web3signer_helper_scripts "$staging_dir"
    
    echo "✓ Configuration files created"
    
    # Test and start services in staging directory
    echo "Starting Web3signer services..."
    cd "$staging_dir"
    ...
    set +e  # Temporarily disable error exit for this section  
    ...
    set +e  # Temporarily disable error exit for migration
    docker compose up flyway 2>&1 | sed 's/^/  /'
    ...
    if [[ $flyway_exit_code -eq 0 ]]; then
        ...
        echo "✗ Database migrations failed (exit code: $flyway_exit_code)"
        echo "Checking migration logs:"
        docker compose logs flyway --tail=10 | sed 's/^/  /'
        return 1
    fi
    
    echo
    echo -e "${UI_MUTED}Starting Web3signer service...${NC}"
    
    # More aggressive cleanup before starting Web3signer
    docker ps -aq --filter "publish=${web3signer_port}" | xargs -r docker rm -f 2>/dev/null || true
    docker ps -aq --filter "expose=${web3signer_port}" | xargs -r docker rm -f 2>/dev/null || true
    
    sleep 3  # Give Docker more time to release port bindings
    ...
    # Temporarily disable error trap and error exit for Web3signer startup section
    trap - ERR
    set +e
    
    # Try starting Web3signer with retry logic
    local web3signer_attempts=0
    local web3signer_started=false
    
    while [[ $web3signer_attempts -lt 3 ]]; do
        ((web3signer_attempts++))
        
        local compose_output=$(docker compose up -d web3signer 2>&1)
        local web3signer_result=$?
        echo "$compose_output" | sed 's/^/  /'
        
        if [[ $web3signer_result -eq 0 ]]; then
            web3signer_started=true
            break
        else
            echo "  Attempt $web3signer_attempts failed with exit code $web3signer_result"
            echo "  Full error output:"
            echo "$compose_output" | sed 's/^/    /'
            if [[ $web3signer_attempts -lt 3 ]]; then
    ...
    if [[ "$web3signer_started" != "true" ]]; then
        echo "✗ Failed to start Web3signer after 3 attempts"
        # Re-enable error handling before returning
        set -e
        trap cleanup_failed_installation ERR INT TERM
        return 1
    fi
    ...
    # Error handling already disabled from retry section
    local attempts=0
    local max_attempts=20  # Increased from 10 to 20 (60 seconds total)
    ...
    # ATOMIC MOVE: All components working, now commit the installation
    ...
    echo "Moving from staging to final location..."
    ...
    if import_choice=$(fancy_select_menu "Do you want to start the key import process?" "${import_options[@]}"); then
        case $import_choice in
            0)
                ...
                echo
                # First, we need to set the keystore password in .env
                ...
                if [[ -z "$keystore_password" ]]; then
                    ...
                    sed -i "s|KEYSTORE_PASSWORD=.*|KEYSTORE_PASSWORD=${keystore_password}|g" "$service_dir/.env"
                    echo "✓ Keystore password configured"
                    echo
                    
                    # Now call the key import script
                    if [[ -f "$service_dir/import-keys.sh" ]]; then
                        cd "$service_dir" && ./import-keys.sh
                        echo
...
install_vero() {
    ...
    for dir in "$HOME"/ethnode*; do
        if [[ -d "$dir" && -f "$dir/.env" ]]; then
            ...
            # Check if network actually exists in Docker
...
if [ -n "$1" ]; then
    ...
    echo
    echo "Recommended: Use USB drive for validator keys (safer than local storage)"
    echo
    echo "Common keystore locations:"
    echo "  - USB drive: /media/usb/validator_keys (RECOMMENDED)"
    echo "  - USB drive: /mnt/validator-usb/validator_keys"
    echo "  - Local: ~/validator_keys (NOT recommended - keys stay on machine)"
    echo "  - Custom path: /path/to/your/validator_keys"
    echo
    read -p "Enter path to validator keystore directory: " KEYSTORE_SOURCE
...
create_vero_env_file() {
    ...
    for ethnode in "${ethnodes[@]}"; do
        ...
        local beacon_client=""
        local network_name="${ethnode}-net"
        
        # Check which beacon client is running (look for containers with beacon API port)
...
remove_web3signer() {
    ...
    docker ps -a --filter "name=web3signer" --format "{{.ID}}" | xargs docker rm -f 2>/dev/null || true
    docker ps -a --filter "name=web3signer-postgres" --format "{{.ID}}" | xargs docker rm -f 2>/dev/null || true
    docker ps -a --filter "name=web3signer-flyway" --format "{{.ID}}" | xargs docker rm -f 2>/dev/null || true
    docker ps -a --filter "name=web3signer-init" --format "{{.ID}}" | xargs docker rm -f 2>/dev/null || true
    
    echo -e "${UI_MUTED}Removing Docker networks...${NC}"
    docker network rm web3signer-net 2>/dev/null || true
    
    # Remove all Web3signer-related volumes
    echo -e "${UI_MUTED}Removing all Web3signer volumes...${NC}"
    docker volume ls --filter "name=web3signer" --format "{{.Name}}" | xargs docker volume rm -f 2>/dev/null || true
    ...
    docker image rm consensys/web3signer:latest 2>/dev/null || true
    docker image rm postgres:15 2>/dev/null || true
    docker image rm flyway/flyway:8 2>/dev/null || true
