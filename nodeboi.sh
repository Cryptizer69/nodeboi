#!/bin/bash
# NODEBOI v1.0.2 - Ethereum Node Automation
set -eo pipefail
...
get_latest_version() {
    ...
    case $client in
        reth)
            ...
            [[ -z "$version" ]] && version="v1.0.2"  # Fallback
            ;;
        besu)
            if [[ -n "$github_token" ]]; then
...
prompt_version() {
    ...
    case "$version_choice" in
        ...
        2)
            while true; do
                read -r -p "Enter version (e.g., v1.0.2 or 25.7.0): " selected_version
